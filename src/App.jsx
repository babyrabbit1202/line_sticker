...
function App() {
  ...
  // 根據選擇的模型決定圖片生成模型
  const imageModelName = modelName === 'gemini-3-pro-preview' ? 'gemini-3-pro-image-preview' : modelName
  ...
  const handleGenerateCharacter = async () => {
    ...
    try {
      const character = await generateCharacter(apiKey, characterDescription || theme, uploadedCharacterImage, imageModelName)
  ...
  const handleGenerateTextStyle = async () => {
    ...
    try {
      const style = await generateTextStyle(apiKey, theme, characterDescription, modelName)
      setTextStyle(style)
  ...
  const handleGenerateDescriptions = async () => {
    ...
    if (!textStyle.trim()) {
      ...
      try {
        finalTextStyle = await generateTextStyle(apiKey, theme, characterDescription, modelName)
  ...
  const handleSplitGrids = async () => {
    ...
    try {
      ...
      const mainImg = await generateMainImage(apiKey, characterImage, theme, imageModelName)
      ...
      const tabImg = await generateTabImage(apiKey, characterImage, theme, imageModelName)
        ...
        <div className="step-section">
          ...
          <div className="form-group">
            ...
            <select
              ...
              className="form-input"
            >
              <option value="gemini-3-pro-preview">Gemini 3.0 Pro Preview (最新 - 強大)</option>
              <option value="gemini-2.0-pro-exp-02-05">Gemini 2.0 Pro (最新 - 強大)</option>
              <option value="gemini-2.0-flash">Gemini 2.0 Flash (推薦 - 快速)</option>
              <option value="gemini-1.5-flash">Gemini 1.5 Flash (經濟 - 快速)</option>
            </select>
            <p className="form-hint">建議使用 Gemini 3.0 Pro Preview，效果最強大。</p>
          </div>
        </div>
