 ...
 * @param {string} modelName - Gemini 模型名稱
 * @returns {Promise<string>} 角色圖片的 Data URL
 */
export async function generateCharacter(apiKey, theme, uploadedImage = null, modelName = 'gemini-2.0-flash') {
 ...
 */
export async function generateMainImage(apiKey, characterImageDataUrl, theme, modelName = 'gemini-2.0-flash') {
  const genAI = new GoogleGenerativeAI(apiKey)
  const model = genAI.getGenerativeModel({ model: modelName })
 ...
 * @returns {Promise<string>} 生成的圖片 Data URL
 */
export async function generateTabImage(apiKey, characterImageDataUrl, theme, modelName = 'gemini-2.0-flash') {
  const genAI = new GoogleGenerativeAI(apiKey)
  const model = genAI.getGenerativeModel({ model: modelName })
